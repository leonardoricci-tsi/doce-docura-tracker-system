  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        responseContent = `❌ **Erro ao buscar os dados:** ${data.error}`;
      }
...
${data.lotes_producao.length > 0 ? `**Últimos Lotes:**  
...
Como posso ajudá-lo com mais informações específicas?`;
    ...
    if (data.lotes_encontrados || data.produtos_encontrados) {
      ...
      if (data.lotes_encontrados?.length === 0 && data.produtos_encontrados?.length === 0) {
        response = '⚠️ **Não encontrei resultados para sua busca.** Tente usar outros termos.';
      }
...
💵 R$ ${Number(venda.preco_venda).toFixed(2)}`
    ...
    return '✅ **Dados encontrados!** Como posso ajudá-lo a interpretá-los?';
  };
  ...
  return (
    <>
      ...
      <Dialog open={isOpen} onOpenChange={handleClose}>
        <DialogContent className="max-w-md max-h-[600px] bg-white border-brand-brown-800 p-0 overflow-hidden [&_button[data-radix-dialog-close]]:hidden">
